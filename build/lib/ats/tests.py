...
# then the stuck, then the test options.
        self.options = AttributeDict(
            ...
            keep = int(options.get('keep', configuration.options.keep)),
            hideOutput = int(options.get('hideOutput', 
                                     configuration.options.hideOutput)),
            script = '',
            clas = [],
            executable = '',
            directory= '',
...
# Now allow for possibility of no script, or clas as unnamed second positional
        lc = len(fixedargs)
        if lc > 2:
...
# See the coding in management.py that prints atsr.py
    ...
    def set (self, status, message):
        ...
        if status in (PASSED, CREATED, RUNNING, INVALID, BATCHED): 
            return
    ...
    def getOptions (cls):
        """Returns the dictionary of effectuve value of the test options
