...
class BatchMachine (machines.Machine):
    ...
    def addOptions(self, parser): 
        ...
        parser.add_option("--batchNumNodes", action="store", type="int", dest='batchNumNodes', default = 2, help="Batch using this number of nodes.")
        parser.add_option("--constraints", action="store", type="string", dest='constraints', default = None, help = "Batch on this machine.")

        parser.add_option("--batchBank", action="store", type='string', dest='batchBank', default='axcode', help="Batch using this bank")

        parser.add_option( '--gres', action='store', dest='gres', default=None, help="Job requires the specified parallel Lustre file system(s). Valid labels are lscratcha, lscratchb, lscratch1 ... The ignore descriptor can be used for jobs that don't require a parallel file system, enabling them to be scheduled even if there are parallel file system problems. -BATCH")
